...
export const TourDetailPage = () => {
    ...
    const fetchTour = async () => {
      try {
        const res = await fetch(
          `https://tourbackend-rdtk.onrender.com/api/gettourdetails/${id}`
        );
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      if (values.tickets > (tour?.capacity || "")) {
      ...
      try {
        const res = await fetch(
          `https://tourbackend-rdtk.onrender.com/api/addtourrev/${id}`,
          {
            method: "POST",
            headers: {
  ...
  const filterDates = tour
    ? tour.operationDates.filter((d) => new Date(d) >= new Date())
    : [];
                  ...
                  <table className="border w-full max-w-[30rem]">
                    <tbody className="">
                      <tr className="border ">
                        ...
                        <td className="p-3">{tour.dest}</td>
                      </tr>
                      <tr className="border ">
                        <td className="border p-3">Duration</td>
                        <td className="border p-3">{tour.duration}</td>
                      </tr>
                      <tr className="border ">
                        <td className="border p-3">Capacity</td>
                        <td className="border p-3">{tour.capacity}</td>
                      </tr>
                      <tr className="border ">
                ...
                <div className="flex border border-collapse shadow-md rounded-md w-full justify-start items-start max-w-[30rem]">
                  <div className="max-w-[30rem] p-1 flex flex-col items-start rounded-md bg-gray-100">
                    <h1 className="font-semibold text-2xl mb-5 mt-1">
                      Book Tour <div className="h-[1px] bg-black"></div>
                    </h1>
                    <form
