...
const CreateCategory = () => {
  ...
  const addCategory = async (e: React.FormEvent) => {
    ...
    setISButton(true);
    try {
      const res = await fetch(`${URL}/api/addbusinesscategory`, {
        method: "POST",
        headers: {
          ...
          <div className="flex flex-col">
            <div className="flex justify-between flex-wrap gap-y-5">
              <div className="text-black flex flex-col gap-1 ">
                <h1 className="md:text-3xl text-lg font-bold">Add Category</h1>
                <p>Add a Business Category on Toursewa</p>
              </div>
              <button
                onClick={() => navigate("/admin/businesscategory")}
            ...
            >
              ...
              <div className="flex justify-center  w-full">
                <button className=" rounded-lg p-3 w-3/4 md:w-1/3 text-sm text-white bg-button hover:bg-orange-700 ">
                  Add Category {isButton ? <ButtonLoader /> : ""}
                </button>
              </div>
            </form>
          </div>
