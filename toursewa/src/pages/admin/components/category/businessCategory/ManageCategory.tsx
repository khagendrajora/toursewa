...
const ManageCategory = () => {
  ...
  const [filter, setFilter] = useState<BCategory[] | null>([]);
    ...
    try {
      ...
      if (confirmed) {
        ...
        const response = await axios.delete(
          `${URL}/api/deletebusinesscategory/${id}`
        );
        ...
        <div className="flex justify-between flex-wrap gap-y-5">
          <div>
            ...
            <p className="mt-2 text-xs md:text-lg">
              List of Business Categories on Toursewa
            </p>
          </div>
          <Link
            to="/admin/businesscategory/addcategory"
                ...
                <table className="table-auto border-collapse  border border-gray-500 text-xs md:text-lg ">
                  <thead className="bg-neutral-400 text-white">
                    <tr>
                      ...
                      <th className="border font-normal border-gray-500 min-w-[110px] md:min-w-[160px] p-3">
                        Category Name
                      </th>
                      <th className="border font-normal border-gray-500 p-3">
                        Description
                      </th>
                      <th className="border font-normal border-gray-500 min-w-[100px] md:min-w-[150px] p-3">
                        Sub-Category
                      </th>
                      <th className="border font-normal border-gray-500 p-3">
                        Action
                  ...
                  <tbody className="">
                    {filter &&
                      filter.map((category, i) => (
                        <tr key={category._id}>
                          <td className="border font-normal border-gray-500 text-center">
                            {i + 1}
                          </td>
                          <td className="border font-normal border-gray-500 text-center">
                            {category.categoryName}
                          </td>
                          <td className="border font-normal border-gray-500 p-3 text-center">
                          ...
                          <td className="flex justify-between flex-col border font-normal border-b-0  text-white p-2 ">
                            <div className="flex gap-4 md:gap-7">
                              <button
                                className="hover:scale-110"
                                onClick={() =>
                                  navigate(
                                    `/admin/businesscategory/updatecategory/${category.categoryId}`
                                  )
                                }
                                title="Edit"
                              >
                              ...
                              <button
                                ...
                                onClick={() =>
                                  navigate(
                                    `/admin/businesscategory/addbusinesssubcategory/${category.categoryId}`
                                  )
                                }
                              >
                                <FontAwesomeIcon
