...
export const AddBlogs = () => {
  ...
  const [blogsImage, setBlogImages] = React.useState<ImageListType>([]);
  const [title, setTitle] = useState("");
  const [desc, setDesc] = useState("");

  const onImageGallaryChange = async (imageList: ImageListType) => {
    setBlogImages(imageList);
  };
  ...
  const addBlog = async (e: React.FormEvent) => {
    ...
    blogsImage.forEach((image) => {
      formData.append(`blogsImage`, image.file as File);
    });
    try {
      const res = await fetch(
        "https://tourbackend-rdtk.onrender.com/api/addblogs",
        {
          method: "POST",
          body: formData,
        }
          ...
          <div className="flex flex-col">
            <div className="flex justify-between flex-wrap gap-y-5">
              <div className="text-black flex flex-col gap-1 ">
                <h1 className="font-bold text-xl lg:text-2xl">Add Blog</h1>
                <p>Add a Blog on Toursewa</p>
              </div>
              <button
                onClick={() => navigate(-1)}
